...
def headpose_pred_to_degree(pred):
    ...
    # print(type(pred), type(idx_tensor))
    #idx_tensor = torch.FloatTensor(idx_tensor).type_as(pred).to(device)
    idx_tensor = torch.FloatTensor(idx_tensor).type_as(pred)
...
def keypoint_transformation(kp_canonical, he, wo_exp=False):
    ...
    #     yaw_future = executor.submit(headpose_pred_to_degree, yaw)
    #     pitch_future = executor.submit(headpose_pred_to_degree, pitch)
    #     roll_future = executor.submit(headpose_pred_to_degree, roll)

    #     futures = [yaw_future, pitch_future, roll_future]
