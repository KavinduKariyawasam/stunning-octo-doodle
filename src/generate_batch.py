...
def get_data(first_coeff_path, audio_path, device, still=False, idlemode=False, length_of_audio=False, use_blink=True):
    ...
    if idlemode:
        num_frames = int(length_of_audio * 20)
        indiv_mels = np.zeros((num_frames, 80, 16))
    else:
        wav = audio.load_wav(audio_path, 16000) 
        wav_length, num_frames = parse_audio_length(len(wav), 16000, 20)
